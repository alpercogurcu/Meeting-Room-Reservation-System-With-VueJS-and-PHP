    ...
    if ($sql) {
        ...
        if ($toplanti_salonlari->rowCount()) {
            foreach ($toplanti_salonlari as $salon) {
                ...
                $salonbazlisaatler = $db->prepare("Select * from genel_ayar where salon_id=:salonid");
                $salonbazlisaatler->execute(array("salonid"=> $salon["id"]));

                foreach($salonbazlisaatler as $sSaat)
                {
                    $baslangic_saati = $sSaat['baslangic_saati'];
                    $bitis_saati =  $sSaat['bitis_saati'];
                    $hazirlik_suresi = $sSaat['hazirlik_suresi'];
                    $toplanti_suresi = $sSaat['toplanti_suresi'];
                ...
                $salonAyarlari["baslangicSaati"] =$baslangic_saati_time ;
                $salonAyarlari["bitisSaati"] = date('H:i:s', strtotime($bitis_saati));
                $salonAyarlari["hazirlikSuresi"] = $hazirlik_suresi;
                $salonAyarlari["toplantiSuresi"] = $toplanti_suresi;

                $returnArr["salonAyarlari"] = $salonAyarlari;
